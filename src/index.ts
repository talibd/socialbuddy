...
const cancelPostDeclaration: FunctionDeclaration = {
  name: "cancel_post",
...
const editPostDeclaration: FunctionDeclaration = {
...
const reschedulePostDeclaration: FunctionDeclaration = {
...
bot.on('message', async (ctx) => {
  ...
  if (tempMediaUrl) {
    try {
      ...
      else if (contentType.includes('quicktime') || urlLower.endsWith('.mov')) ext = '.mov';
...
If the user attached an image or video, they will pass it as [Attached Media: URL]. Always include this exact URL in the mediaUrls parameter.`
    ...
    while (functionCalls && functionCalls.length > 0) {
      ...
      if (call && call.name === 'schedule_post') {
        ...
        const { postId, newScheduledAt } = call.args as any;
        const post = await prisma.post.findUnique({ where: { id: postId, userId: user.id } });
        if (!post) {
            functionResponseData = { error: "Post not found." };
        } else if (post.status !== 'pending') {
