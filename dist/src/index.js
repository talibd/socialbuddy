...
const getPostingStatsDeclaration = {
...
bot.start(async (ctx) => {
    ...
    await ctx.reply("👋 Welcome to SocialBuddy! I'm your AI Social Media Manager.\n\n" +
...
bot.command('connect', (ctx) => {
    ctx.reply("Which platform would you like to connect?", Markup.inlineKeyboard([
...
bot.action(/connect_(.+)/, async (ctx) => {
    ...
    await ctx.reply(`To connect your <b>${platform}</b> account, please click the link below to authorize SocialBuddy:\n\n🔗 <a href="${authUrl}">Connect ${platform}</a>\n\nIf the link above doesn't work, copy and paste this URL into your browser:\n${authUrl}`, { parse_mode: 'HTML' });
});
